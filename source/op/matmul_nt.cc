...
REGISTER_OP("MatmulNt")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ...
      DimensionHandle output_cols = c->Dim(b, 0);
      DimensionHandle inner_a = c->Dim(a, 1);
      DimensionHandle inner_b = c->Dim(b, 1);
...
class MatmulNtOp : public OpKernel {
  public :
    ...
    void Compute(OpKernelContext* context) override {
        ...
        OP_REQUIRES (context, (a.shape().dim_size(1) == b.shape().dim_size(1)),	errors::InvalidArgument ("dimensions of x and w do not match!"));
        int m = a.shape().dim_size(0);
        int k = a.shape().dim_size(1);
        int n = b.shape().dim_size(0);
        Tensor * output = NULL;
